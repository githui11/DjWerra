...
export async function GET() {
  ...
  if (!projectId) {
    return NextResponse.json({ error: 'Sanity project ID not configured' }, { status: 500 })
  }

  if (!token) {
    return NextResponse.json({
      error: 'SANITY_API_TOKEN not found. Please create a token with Editor permissions at https://sanity.io/manage/project/2ju0iiuz/api and add it to your .env.local file'
    }, { status: 500 })
  }
    ...
    for (const [index, video] of youtubeVideos.entries()) {
      try {
        const doc = {
          ...
          featured: false,
          // Stagger publish dates so they appear in order (oldest to newest)
          publishedAt: new Date(Date.now() - (youtubeVideos.length - index) * 24 * 60 * 60 * 1000).toISOString(),
        }
